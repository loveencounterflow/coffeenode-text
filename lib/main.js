...
(function() {
  var BAP, TRM, TYPES, log, rpr, _fill_in_default_formats, _fill_in_get_matcher, _fill_in_get_method,
    __slice = [].slice;
  ...
  _fill_in_get_method = function(matcher, formats) {
    return function(template, data_or_handler) {
      ...
      return template.replace(matcher, (function(_this) {
        return function($0, $1, $2, $3) {
          var expression, format, formatter, key, value, _ref;
          expression = $2 != null ? $2 : $3;
          if ($2 != null) {
            expression = $2;
            _ref = expression.split(/:/), key = _ref[0], format = _ref[1];
          } else {
            expression = key = $3;
            formatter = null;
          }
          if (handler != null) {
            return handler(null, key, format);
          }
          value = data[key];
          if (value === void 0) {
  ...
  _fill_in_get_matcher = function(activator, opener, closer, seperator, escaper) {
    if (activator == null) {
      activator = '$';
    }
    if (opener == null) {
    ...
    return RegExp("([^" + escaper + "]|^)" + activator + "(?:" + opener + "([^" + opener + closer + "]*)" + closer + "|([^" + activator + opener + closer + seperator + "{}<>()\\|\\s*+.,;:!\"'§$%&/=?`´\\#]+))", "g");
  };

  _fill_in_default_formats = {
    'quoted': function(value) {
      return '"' + (TYPES.isa_text(value) ? value : rpr(value)) + '"';
    }
  };

  this.fill_in = _fill_in_get_method(_fill_in_get_matcher(), _fill_in_default_formats);

  this.fill_in.get_matcher = _fill_in_get_matcher;

  this.fill_in.formats = _fill_in_default_formats;


  /* TAINT use options argument */

  this.fill_in.create = function(formats, activator, opener, closer, seperator, escaper) {
    var R, matcher;
